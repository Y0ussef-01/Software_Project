...
const registerAttendance = async (req, res) => {
    try {
        ...
        if (deviceUsedBefore && deviceUsedBefore.student !== studentId) {
            return res.status(403).json({ message: "Device already used by another student today" });
        }
...
const updateProfileImg = async (req, res) => {
    try {
        ...
        res.json({ message: "Profile image updated", student: updatedStudent });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
};
...
const registerCourse = async (req, res) => {
    try {
        ...
        if (!courseId || !groupName) {
            return res.status(400).json({ message: "Course Id and GroupName are required" });
        }
        const student = await Student.findById(studentId);
        ...
        if (alreadyRegistered) return res.status(400).json({ message: "Course already registered" });
        ...
        if (student.hours + course.hours > student.maxHours) {
            return res.status(400).json({ message: `Cannot exceed maximum limit of ${student.maxHours} hours` });
        }
        ...
        for (let group of groupsToRegister) {
            if (group.enrolledStudents.length >= group.capacity) {
                return res.status(400).json({ message: `Group ${group.groupName} is full` });
            }
        }
        ...
        await student.save();
        res.status(200).json({ message: "Course registered successfully", hours: student.hours });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
};
...
const dropCourse = async (req, res) => {
    try {
        ...
        for (let groupId of groupsToDrop) {
            await Group.findByIdAndUpdate(groupId, { $pull: { enrolledStudents: studentId } });
        }
...
const getMyGrades = async (req, res) => {
    try {
        const studentId = req.user.id;
        const student = await Student.findById(studentId).select('registeredCourses').populate({
            path: 'registeredCourses.course', select: 'name'
        });
        if (!student) {
            return res.status(404).json({ message: "Student not found" });
        }
        ...
        res.status(200).json({ message: "Degrees of Student", grades: gradesDetails });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
};
