...
export default function TeacherCoursesTable({
  courses, onEditClick, onDeleteClick, isLoading, allCourses,
}) {
  ...
  const getGroupData = (courseId, groupName) => {
    if (!courseId || !groupName || !allCourses) return null;
        ...
        <Table sx={{ minWidth: 750 }}>
          <TableHead>
            <TableRow sx={{
              ...
              {["Course Code", "Course Name", "Group", "Schedule", "Location", "Actions"].map((h, i) => (
                <TableCell key={h}
                  align={i === 5 ? "right" : "left"}
                  sx={{ fontWeight: 700, color: theme.palette.text.primary, borderBottomColor: theme.palette.divider }}
                >
                  {h}
                </TableCell>
              ))}
          ...
          <TableBody>
            {groupedArray.map((courseGroup, groupIndex) =>
              courseGroup.groups.map((group, rowIndex) => {
                ...
                const groupData = getGroupData(group.courseId, group.groupName);
                const appointment = groupData?.appointment || null;
                const room = groupData?.Room || groupData?.room || "";
                const type = groupData?.type || "";

                
                const shortGroup = group.groupOnly?.replace(/-(Lecture|Lab|Tutorial)$/i, "") || group.groupOnly;
                ...
                return (
                  <TableRow
                    ...
                    sx={{
                      backgroundColor: groupIndex % 2 === 0
                        ? "transparent"
                        : isDark ? "rgba(255,255,255,0.02)" : "rgba(21,43,72,0.02)",
                      "&:hover": {
                        backgroundColor: isDark ? "rgba(255,255,255,0.05)" : "rgba(21,43,72,0.05)",
                      },
                    }}
                  >
                    ...
                    {rowIndex === 0 && (
                      <TableCell rowSpan={courseGroup.groups.length} sx={{
                    ...
                    {rowIndex === 0 && (
                      <TableCell rowSpan={courseGroup.groups.length} sx={{
                        fontSize: "0.95rem", verticalAlign: "middle",
                        borderRight: `1px solid ${theme.palette.divider}`,
                      }}>
                      ...
                      <Box sx={{ display: "flex", flexDirection: "column", gap: 0.3 }}>
                        ...
                        {type && (
                          <Typography variant="caption" sx={{
                            color: type === "Lecture" ? "primary.main" : "success.main",
                            fontWeight: 600,
                          }}>
                            {type}
                          </Typography>
                    ...
                    <TableCell>
                      {appointment ? (
                        <Box sx={{ display: "flex", flexDirection: "column", gap: 0.4 }}>
                          ...
                          <Box sx={{ display: "flex", alignItems: "center", gap: 0.8, ml: 0.3 }}>
                            <AccessTimeIcon sx={{ fontSize: 13, color: "text.secondary" }} />
                            <Typography variant="caption" color="text.secondary">
                              {appointment.startTime || "TBA"} - {appointment.endTime || "TBA"}
                            </Typography>
                          </Box>
                        </Box>
                      ) : (
